...
class FirebaseService {
  ...
  async syncMessage(message: EmergencyMessage) {
    try {
      const encryptedContent = encryptMessage(message.content || message.payload || '');
      ...
      const cloudMsg: CloudMessage = {
        ...
        timestampCreated: message.timestampCreated || (message.createdAt ? new Date(message.createdAt).getTime() : Date.now()),
      };

      const db = getFirestore();
      const docRef = doc(db, MESSAGES_COLLECTION, message.id);
      await setDoc(docRef, cloudMsg, { merge: true });
