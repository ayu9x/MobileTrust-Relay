...
export const MessageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    AsyncStorage.setItem('@messages', JSON.stringify(messages)).catch((e: unknown) => 
      console.error('Failed to save messages', e)
    );
  }, [messages]);

  const addMessage = async (msg: EmergencyMessage): Promise<boolean> => {
    const isDuplicate = await FirebaseService.isDuplicate(msg.recipient, msg.content || msg.payload || '');
